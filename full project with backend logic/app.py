...
# update userdata set messagehistory = '{"defaultmessage": "No new message history"}' where id = 1;
...
#     print(f"Database '{servicedb}' created and connected successfully.")
...
#     cursor.execute(initialization)
#     print("Table 'services' created successfully (if it didn't exist).")

#     connection.commit()
#     connection.close()
#     print("Connection closed.")

# # INSERT INTO services (service, service, time, price, discount, note)
# # VALUES
# #     ('Classic Full Set', 2, 100, 0.0, ''),
# #     ('Hybrid Full Set', 2, 125, 0.0, ''),
# #     ('Volume Full Set', 2, 135, 0.0, ''),
# #     ('Mega Volume Full Set', 2, 150, 0.0, ''),
# #     ('Lash Lift & Tint', 1, 85, 0.0, '(15% Off)');
...
try:
    connection = sqlite3.connect(reservationdb)
    print(f"Database '{reservationdb}' created and connected successfully.")

    cursor = connection.cursor()
    initialization = """
        CREATE TABLE IF NOT EXISTS reservations (
          reservation_id INTEGER PRIMARY KEY AUTOINCREMENT,
          reservation_customer_id INTEGER NOT NULL,
          reservation_service VARCHAR(255) NOT NULL,
          reservation_datetime TEXT NOT NULL UNIQUE,
          reservation_duration NUMERIC NOT NULL,
          reservation_price NUMERIC NOT NULL,
          reservation_discount REAL DEFAULT 0.0,
...
# INSE INTO reservations (reservation_customer_id, reservation_service, reservation_datetime, reservation_duration, reservation_price, reservation_discount, reservation_note)
# VALUES
#     ('1', 'Classic Full Set', '2026-07-31 14:30:00', 2, 24, 0.0, 'no note');RT
...
def pull_existing_reservations():
    with sqlite3.connect(reservationdb) as connection:
        cursor = connection.cursor()
        ...
        for result in results:
            result = {
                ...
                "reservation_customer_id": result[1],
                "reservation_service": result [2],
                "reservation_datetime": result[3],
                "reservation_duration": result[4],
                "reservation_price": result[5],
                "reservation_discount": result[6],
                "reservation_note": result[7],
                # "reserved_at": datetime(result[8]).astimezone(ZoneInfo("US/Central"))
                "reserved_at": result[8]
            }
            existing_reservations.append(result)
        return existing_reservations

# utc_time.astimezone(ZoneInfo("US/Central"))
...
@app.route('/reservation_availability', methods=['GET', 'POST'])
def display_existing_reservations():
    existing_reservations = pull_existing_reservations()
...
def ig():
    return render_template("ig2.html")
...
def save_to_database(updatedata):
    ...
    try:
        ...
        if updatedata.get("registration"):
            ...
            # print(updatedata)
            # SQL statement with placeholders
            # sql = "UPDATE userdata SET messagehistory = ? WHERE id = 1"
...
def generate_unique_code(subject, maxdigits=8):
    while True:
        code = ""
        for _ in range(subject):
            code += random.choice(ascii_uppercase)
            code = str(code)[:maxdigits]
        return code
...
def room():
    ...
    # print(f"sessionuser4: {messagehistory}")
    # if not status or messagehistory is None:
    #     nonuserstatus = {"status": False, "error": "Please sign in or create an account to chat with us."}
    #     return nonuserstatus
