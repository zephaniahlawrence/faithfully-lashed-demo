...
export function initiatechat(messagedata){
    ...
    const createmessage = (status, name, message) => {
        ...
        for (const chatmessagesdisplay of chatmessagesdisplays) {
            // console.log(`chatmessagesdisplay: ${chatmessagesdisplay.outerHTML}`);
            const lastitem = chatmessagesdisplay.lastElementChild;
            lastitem.scrollIntoView({ behavior: 'smooth', block: 'end'});
            // // if (chatmessagesdisplay.scrollHeight > 0) {
            //     // console.log(`scrollHeight: ${chatmessagesdisplay.scrollHeight}`);
            //     chatmessagesdisplay.scrollTop = chatmessagesdisplay.scrollHeight;
            //     // console.log(`scrollHeight2: ${chatmessagesdisplay.scrollTop}`);
            //     // console.log({
            //     //     scrollHeight: chatmessagesdisplay.scrollHeight,
            //     //     clientHeight: chatmessagesdisplay.clientHeight,
            //     //     offsetHeight: chatmessagesdisplay.offsetHeight,
            //     //     overflowY: getComputedStyle(chatmessagesdisplay).overflowY
            //     // });
            //     // setTimeout(() => {
            //     //     console.log("100ms later:", chatmessagesdisplay.scrollTop);
            //     // }, 100);
            //     // break;
            // // }
        }
    ...
    Object.values(userdata).forEach(user => {
        ...
        Object.values(messagehistory).forEach(chatroom => {
            if (chatroom["roomstatus"]) {
                ...
                // if (adminchatitem.id != "fl0000") {
                //     update_class_chatrooms(existingchatroomforms, 'prepend', adminchatitem);
                // }
                // else {
                //     update_class_chatrooms(existingchatroomforms, 'append', adminchatitem);
                // }
            }
        });
    });
